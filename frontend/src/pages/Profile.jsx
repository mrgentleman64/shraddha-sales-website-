    ...
    <div className="page-shell container mx-auto px-4 py-10">
      <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="section-title mt-2">Hi, {user.name}</h1>
        ...
        <Tabs defaultValue="orders">
          ...
          <TabsContent value="orders">
            <div className="mt-6 space-y-4">
              {orders.length === 0 ? (
                <div className="rounded-3xl bg-slate-50 p-8 text-center text-sm text-slate-500">No orders yet.</div>
              ) : (
                orders.map((order) => (
                  <div key={order.id} className="rounded-3xl border border-slate-200 bg-white/70 p-5">
